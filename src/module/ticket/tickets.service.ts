...
export class TicketsService {
  ...
  async createTicket(dto: CreateTicketDto, adminId: string) {
    this.audit(adminId, 'INQUIRY', dto.ticketId, 'CREATE', undefined, {
      subject: dto.subject,
    });
  ...
  async getDashboardStats() {
    ...
    counts.forEach((item) => {
      ...
      if (
        item.status === InquiryStatus.REVIEWED ||
        item.status === InquiryStatus.CONTACTED ||
        item.status === InquiryStatus.QUALIFIED
      ) {
  ...
  async updateTicket(id: string, dto: UpdateTicketDto, adminId: string) {
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      const updatedTicket = await tx.contactInquiry.update({
        ...
        data: {
          ...
          tags:
            dto.tags !== undefined ? dto.tags : (existingTicket.tags as any),

          adminNotes: dto.replyMessage
            ? dto.replyMessage
            : existingTicket.adminNotes,
          respondedAt: dto.replyMessage
            ? new Date()
            : existingTicket.respondedAt,
          closedAt: dto.status
    ...
    this.audit(
      ...
      id,
      'UPDATE',
      { status: existingTicket.status, priority: existingTicket.priority },
